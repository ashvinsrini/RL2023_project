...
class OrnsteinUhlenbeckProcess:
    """
    mu: Represents the long-term mean of the noise. In the context of exploration in reinforcement learning, it helps the agent explore around a particular value. Setting it to 0 means that the noise will oscillate around 0.
    theta: Represents the speed of mean reversion or how quickly the noise reverts to the mean (mu). A higher value of theta means the noise will revert to the mean faster.
    sigma: Represents the volatility or amplitude of the noise. A higher sigma will result in noisier actions with larger deviations from the mean.
    """
    ...
    def sample(self):
        ...
        self.state = x + dx
        return self.state.astype(np.float32)  # Convert to NumPy array
...
class DDPGAgent(BaseAgent):
    def __init__(self, config=None):
        ...
        # print ("config", config)
        # print ("config", config['theta'], "config.sigma", config['sigma'])
        # print ("self.cfg.theta", self.cfg.theta)
        # Ornstein-Uhlenbeck Process for exploration noise
        self.exploration_noise = OrnsteinUhlenbeckProcess(size=self.action_dim, theta=config['theta'], sigma=config['sigma'])
    ...
    def get_action(self, observation, evaluation=False):
        ...
        if (self.buffer_ptr < self.random_transition) and (evaluation == False): # collect random trajectories for better exploration.
            ...
            #pdb.set_trace()
            # if evaluation equals False, add normal noise to the action, where the std of the noise is expl_noise
            # Add Ornstein-Uhlenbeck noise during exploration
            if not evaluation:
                action += torch.from_numpy(self.exploration_noise.sample()).to(self.device)
    ...
    def train_iteration(self):
        ...
        while not done:
            ...
            # Add reset of Ornstein-Uhlenbeck process at the end of each episode
            if done:
